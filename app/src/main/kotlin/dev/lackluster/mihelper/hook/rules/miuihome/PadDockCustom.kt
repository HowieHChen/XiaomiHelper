...
object PadDockCustom : YukiBaseHooker() {
    private val dockDuration by lazy {
        Prefs.getInt(PrefKey.HOME_PAD_DOCK_TIME_DURATION, 180)
    }
    private val dockSafeHeight by lazy {
        Prefs.getInt(PrefKey.HOME_PAD_DOCK_SAFE_AREA_HEIGHT, 300)
    }
    override fun onHook() {
        if (dockDuration != 180 && Device.isPad) {
            "com.miui.home.recents.DockGestureHelper".toClass()
                .method {
        ...
        if (dockSafeHeight != 300 && Device.isPad) {
            "com.miui.home.launcher.dock.DockTouchHelper".toClass()
                .method {
