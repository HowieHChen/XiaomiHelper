...
object ClearButton : StaticHooker() {
    private val clearButton by Preferences.MiuiHome.RECENT_CLEAR_BUTTON.lazyGet()
    ...
    override fun onInit() {
        updateSelfState(clearButton != 0)
    }

    override fun onHook() {
        if (clearButton == 1) {
            val metIsInMultiWindowModeCompatAndroidT = "com.miui.home.launcher.DeviceConfig".toClassOrNull()?.resolve()?.firstMethodOrNull {
            ...
            clzRecentsContainer?.apply {
                resolve().firstMethodOrNull {
                    ...
                    metIsInMultiWindowModeCompatAndroidT?.invoke(null)?.let {
                        result(!it)
                    } ?: result(proceed())
                }
            }
        } else if (clearButton == 2) {
            clzRecentsContainer?.apply {
