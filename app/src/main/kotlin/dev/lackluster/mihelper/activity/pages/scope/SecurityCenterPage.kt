...
class SecurityCenterPage : BasePage() {
    override fun getTitle(): String {
        return activity.getString(
            if (Device.isPad)
                R.string.scope_security_center_pad
            else
                R.string.scope_security_center
        )
    }
    override fun onCreate() {
        TitleText(textId =
            if (Device.isPad) R.string.scope_security_center_pad
            else R.string.scope_security_center
        )
        ...
        TextSummaryWithSwitch(
            TextSummaryV(
                textId = R.string.security_security_bubble_restriction,
            ),
            SwitchV(PrefKey.SECURITY_BUBBLE_RESTRICTION)
        )
        TextSummaryWithSwitch(
            TextSummaryV(
                textId = R.string.cleaner_security_disable_risk_app_notif,
            ),
            SwitchV(PrefKey.SECURITY_NO_RISK_APP_NOTIFICATION)
        )
        TextSummaryWithSwitch(
            TextSummaryV(
                textId = R.string.cleaner_security_remove_report,
            ),
            SwitchV(PrefKey.SECURITY_REMOVE_REPORT)
        )
        TextSummaryWithSwitch(
            TextSummaryV(
                textId = R.string.security_security_system_app_wifi,
            ),
        ...
        TextSummaryWithSwitch(
            TextSummaryV(
                textId = R.string.others_settings_unlock_custom_refresh,
            ),
            ...
            TextSummaryV(
                ...
                onClickListener = {
                    try {
                        ...
                        makeText(activity, activity.getString(R.string.security_power_battery_optimization_failed_toast), LENGTH_LONG).show()
                    }
                })
        )
    }
